# The Codex v2.0 — Operational Canon Edition
...
**Branch:** main  
**Supersedes:** The Codex v1.9 — Operational Canon Edition
...
When an open doorway reveals a connected room, the visible space beyond the threshold must be derived from the Codex definition of the connected room.

The assistant shall not invent the appearance of the connected room from generic fantasy assumptions.
...
The connected room should therefore display the visual identity, architecture, atmosphere, furnishings, symbols, compass-fixed features, and canonical layout elements defined for that room.

An open doorway should function as a partial render of the connected room.

Examples:

Entry Hall → Small Library:

- The visible room beyond the North Door should contain recognizable Small Library features derived from the Small Library render prompt, including bookshelves, reading tables, lecterns, reference collections, lamps, wood-softened stone architecture, and other canonical Small Library elements. The position of the doors should be respected.

Garden → Entry Hall:

- The visible room beyond the Garden's east door should contain recognizable Entry Hall features derived from the Entry Hall render prompt, including side tables, coat hooks, travel bags, lanterns, vines, and the narrow arrival-passage layout.

Foundational Door → Great Hall:

- The visible room beyond the threshold should contain recognizable Great Hall features derived from the Great Hall render prompt, including portions of the Great Knowledge Compass, Palace Sigil, compass-fixed door system, domain medallions, and navigation architecture.

The visible room beyond a doorway should never appear as a generic room of the same genre when a canonical room definition exists.

Non-canonical mistakes:

- Generic fantasy room visible through an open doorway.
- Connected room rendered without reference to its Codex definition.
- Missing canonical identifying features of the connected room.
- A doorway view that contradicts the connected room's render prompt.
...
When rendering a connected room through an open doorway, the assistant should first place the canonical doors and navigational structure defined by the Codex and then populate the remaining visible space with the room's furnishings and decorative features.

Examples:

Entry Hall → Small Library:

- Viewing the Small Library through the Entry Hall North Door should preserve the Small Library's canonical north-south linear layout.
- The Small Library North Door toward the Foundational Door / Great Hall should remain present and recognizable.
- Bookshelves, tables, lecterns, and collections may surround the space but may not replace or obscure the canonical North Door.

Foundational Door → Great Hall:

- The visible Great Hall should preserve compass-fixed openings and navigational architecture.
- Domain features may not replace canonical openings.

Non-canonical mistakes:

- Bookshelf replacing the Small Library North Door.
- Furniture blocking a canonical doorway.
- Decorative features replacing navigational architecture.
- Connected-room topology differing from Codex-defined topology.
- Open-door views that preserve room atmosphere but violate room structure.
...
When a room is viewed through an open doorway, the visible navigational structure of that room should remain legible.

Examples:

- Entry Hall: a clear path should exist between the South Door, North Door, and West Garden Door.
- Small Library: a clear north-south path should exist between the Entry Hall door and the Foundational Door.
- Great Hall: compass-based movement between the Great Knowledge Compass and visible openings should remain readable.
- Antechambers: primary routes between the Great Hall door and realm-facing doors should remain clear.

Non-canonical mistakes:

- Furniture blocking a canonical route.
- Tables occupying the primary circulation path.
- Decorative displays obscuring navigational flow.
- Visual compositions that make room connectivity difficult to understand.
- Room furnishings taking precedence over canonical movement through the space.
...
When constructing a doorway view using the Open Door Room Derivation Protocol, the assistant should:
...
When a station is visible through an open doorway, it should remain subordinate to the room's navigational structure, canonical doors, and primary architectural composition. A doorway view should not become a station render unless the user explicitly requests the station.

Examples:

Entry Hall → Small Library:

- The Instruction Manual Station is located in the northeast corner of the Small Library.
- If the northeast corner falls within the visible doorway view, the Instruction Manual and Guest Book should be visible as a recognizable secondary feature.

Garden → Buddha Statue Station:

- The Buddha Statue remains visible when the northwest corner is visible according to existing Garden rules.

Non-canonical mistakes:

- A visible station disappearing without architectural justification.
- A room render omitting a station that lies within the visible field of view.
- Replacing a station with generic furnishings.
- Contradicting a station's canonical location.
- Allowing a station to dominate a doorway view when the user did not request a station render.
...
Non-canonical mistakes: broad ceremonial chamber, grand scale, missing human welcome details, more than three doors, Garden door anywhere except west, hub-like layout, open north door without Small Library visible beyond, missing visible west-wall Garden door in a north-facing render, connected Small Library rendered as a generic library, bookshelf replacing the Small Library's north door, table blocking the Small Library circulation path, missing visible station when the station's corner is visible.
...
Open door rule: if the Garden east-wall door is visible and open, the Entry Hall should be visible beyond the threshold with spatial continuity and derived from the Entry Hall Codex definition.
...
Non-canonical mistakes: extra exits, hub-like room, display book in center, book blocking north door, treating display book as Codex, grand or monumental scale, station rendered as full-room overview, reading table blocking the north-south path, shelves replacing a canonical door.
...
Active Codex: Codex v2.0 — Operational Canon Edition
...
# 18. v2.0 Change Summary

Codex v2.0 incorporates rendering refinements discovered through Entry Hall north-facing render tests:

1. Adds Open Door Room Derivation Protocol.
2. Adds Topology Preservation Rule.
3. Adds Navigational Path Preservation Protocol.
4. Adds Station Visibility Inheritance Protocol.
5. Revises Entry Hall render requirements to require west-wall Garden Door visibility in north-facing renders.
6. Revises Small Library rendering guidance to preserve the north-south path and prevent furnishings from replacing canonical doors.
7. Clarifies that doorway views inherit connected-room Codex definitions, topology, paths, and visible stations.
