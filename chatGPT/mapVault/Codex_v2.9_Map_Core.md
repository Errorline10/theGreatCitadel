...
Supersedes: /chatGPT/mapVault/Codex_v2.8_Map_Core.md
...
All maps, room diagrams, navigation charts, architectural layouts, room connections, rendering audits, room blueprints, 360 references, and door renders derive from this coordinate system.
...
On all canonical maps and blueprints:
...
When two rooms are not canonically connected, map prompts, blueprint prompts, 360 panorama prompts, room render prompts, and door-render prompts must explicitly state that no doorway, corridor, shared wall, bridge, tunnel, path, label-line connection, visual connector, or architectural connection may exist between them.
...
For maps and blueprints, the Garden and Media Antechamber must remain visibly separated by empty map space unless a future Codex release explicitly defines a canonical connection.
...
All maps, diagrams, room connections, doorway views, navigation systems, room placement logic, room blueprints, 360 panoramas, and door renders derive from this compass geometry.
...
When rendering a map or blueprint:
...
A map or blueprint showing the Garden separated from the Entry Hall by any intermediate structure is non-canonical.

A map or blueprint showing the Garden connected or visually adjacent to Media Antechamber or Great Hall is non-canonical.
...
Every room definition should be simple enough that a map, blueprint, 360 panorama, and door-render reference can be generated directly from compass orientation, door locations, connected rooms, canonical topology, and explicit topology separation rules without requiring interpretation.

If a room definition cannot be converted into a map or blueprint, the room definition is incomplete.

---

# Topology Preservation Rule Expanded

Maps, room renders, doorway views, connected-room views, navigation diagrams, 360 panoramas, blueprints, and door renders must all describe the same underlying architecture.

A room render may show only part of a room. A map may show all of a room. A 360 panorama may show the full room. None may contradict the others.
...
Whenever a new room, antechamber, realm, station, path, map, connection, room reference, blueprint, or door render is added to the Codex, its canonical compass bearing and relationship to existing spaces should be explicitly defined.

Rooms should be added in a manner that allows deterministic map generation, blueprint generation, 360 reference generation, and door-render generation.

End of Codex v2.9 — Map Core.
